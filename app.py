...
def dashboard():
    ...
    cursor.execute('SELECT receiver_phone, amount, timestamp FROM transactions WHERE sender_phone = %s ORDER BY timestamp DESC',
                   (user[2],))
...
def send_money():
    sender_phone = request.form.get('sender_phone')  # Sender's phone from form input
    receiver_phone = request.form.get('receiver_phone')  # Receiver's phone from form input
    amount = request.form.get('amount')  # Amount from form input
    ...
    # Perform transaction (deduct from sender, add to receiver)
